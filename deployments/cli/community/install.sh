...
SCRIPT_DIR=$PWD
SERVICE_FOLDER=planner-app
PLANNER_INSTALL_DIR=$PWD/$SERVICE_FOLDER
export APP_RELEASE=stable
export DOCKERHUB_USER=artifacts.planner.oneworkspacex.com/kik-doctor
export PULL_POLICY=${PULL_POLICY:-if_not_present}
export GH_REPO=kik-doctor/planner
...
mkdir -p $PLANNER_INSTALL_DIR/archive
DOCKER_FILE_PATH=$PLANNER_INSTALL_DIR/docker-compose.yaml
DOCKER_ENV_PATH=$PLANNER_INSTALL_DIR/planner.env
...
function initialize(){
    ...
    local IMAGE_NAME=kik-doctor/planner-proxy
    ...
    if [ $? -eq 0 ]; then
        echo "Planner supports ${CPU_ARCH}" >&2
        echo "available"
        return 0
    else
        echo "" >&2
...
function syncEnvFile(){
    ...
    if [ -f "$PLANNER_INSTALL_DIR/planner.env.bak" ]; then
        updateCustomVariables
        
        # READ keys of planner.env and update the values from planner.env.bak
        ...
        do
            ...
            value=$(getEnvValue "$key" "$planner_INSTALL_DIR/planner.env.bak")
            if [ -n "$value" ]; then
...
function buildYourOwnImage(){
    ...
    export DOCKERHUB_USER="myplanner"
    export APP_RELEASE="local"
    export PULL_POLICY="never"
    CUSTOM_BUILD="true"

    # checkout the code to ~/tmp/planner folder and build the images
    local PLANNER_TEMP_CODE_DIR=~/tmp/planner
    rm -rf $PLANNER_TEMP_CODE_DIR
    mkdir -p $PLANNER_TEMP_CODE_DIR
    REPO=https://github.com/$GH_REPO.git
    git clone "$REPO" "$PLANNER_TEMP_CODE_DIR"  --branch "$BRANCH" --single-branch --depth 1

    cp "$PLANNER_TEMP_CODE_DIR/deployments/cli/community/build.yml" "$planner_TEMP_CODE_DIR/build.yml"

    cd "$PLANNER_TEMP_CODE_DIR" || exit
...
function download() {
    ...
    cd $SCRIPT_DIR
    TS=$(date +%s)
    if [ -f "$PLANNER_INSTALL_DIR/docker-compose.yaml" ]
    then
        mv $PLANNER_INSTALL_DIR/docker-compose.yaml $PLANNER_INSTALL_DIR/archive/$TS.docker-compose.yaml
    fi
    ...
    if [ "$STATUS" -eq 200 ]; then
        echo "$BODY" > $PLANNER_INSTALL_DIR/docker-compose.yaml
    else
        ...
        if [ "$STATUS" -eq 200 ]; then
            ...
            mv $PLANNER_INSTALL_DIR/archive/$TS.docker-compose.yaml $PLANNER_INSTALL_DIR/docker-compose.yaml
            exit 1
        fi
    fi
    ...
    if [ "$STATUS" -eq 200 ]; then
        echo "$BODY" > $PLANNER_INSTALL_DIR/variables-upgrade.env
    else
        # Fallback to download from the raw github url
    ...
    then
        cp "$DOCKER_ENV_PATH" "$PLANNER_INSTALL_DIR/archive/$TS.env"
        cp "$DOCKER_ENV_PATH" "$PLANNER_INSTALL_DIR/planner.env.bak"
    fi

    mv $PLANNER_INSTALL_DIR/variables-upgrade.env $DOCKER_ENV_PATH
    ...
    echo ""
    echo "Most recent version of Planner is now available for you to use"
    echo ""
    echo "In case of 'Upgrade', please check the 'planner.env 'file for any new variables and update them accordingly"
    echo ""
}
function startServices() {
    ...
    if [ -n "$migrator_container_id" ]; then
        ...
        if [ $migrator_exit_code -ne 0 ]; then
            echo "Planner Server failed to start ❌"
            # stopServices
            echo
    ...
    echo "   Planner Server started successfully ✅"
    echo ""
...
function upgrade() {
    ...
    echo "Upgrading Planner to the latest release..."
    echo ""
...
function viewLogs(){
    ...
    then
        ...
        then
            ...
            case $DOCKER_SERVICE_NAME in
                ...
                8) viewSpecificLogs "planner-redis";;
                9) viewSpecificLogs "planner-db";;
                10) viewSpecificLogs "planner-minio";;
                11) viewSpecificLogs "planner-mq";;
                0) askForAction;;
                *) echo "INVALID SERVICE NAME SUPPLIED";;
            esac
        fi
    ...
    then
        ...
        case $ARG_SERVICE_NAME in
            ...
            redis) viewSpecificLogs "planner-redis";;
            postgres) viewSpecificLogs "planner-db";;
            minio) viewSpecificLogs "planner-minio";;
            rabbitmq) viewSpecificLogs "planner-mq";;
            *) echo "INVALID SERVICE NAME SUPPLIED";;
        esac
    else
        echo "INVALID SERVICE NAME SUPPLIED"
...
function backupData() {
    ...
    local BACKUP_FOLDER=$PLANNER_INSTALL_DIR/backup/$datetime
    ...
    backup_container_dir "$BACKUP_FOLDER" "planner-db" "/var/lib/postgresql/data" "pgdata" || exit 1
    backup_container_dir "$BACKUP_FOLDER" "planner-minio" "/export" "uploads" || exit 1
    backup_container_dir "$BACKUP_FOLDER" "planner-mq" "/var/lib/rabbitmq" "rabbitmq_data" || exit 1
    backup_container_dir "$BACKUP_FOLDER" "planner-redis" "/data" "redisdata" || exit 1
...
if [ -f "$DOCKER_ENV_PATH" ]; then
    ...
    if [ -z "$DOCKERHUB_USER" ]; then
        DOCKERHUB_USER=artifacts.planner.oneworkspacex.com/kik-doctor
        updateEnvFile "DOCKERHUB_USER" "$DOCKERHUB_USER" "$DOCKER_ENV_PATH"
    fi
