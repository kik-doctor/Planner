...
function restoreData() {
    ...
    AIRGAPPED_INSTALL_PATH="$HOME/plannerairgapped"
    ...
    if command -v jq &> /dev/null; then
        dockerServiceStatus=$($COMPOSE_CMD ls --filter name=planner-airgapped --format=json | jq -r .[0].Status)
    else
        dockerServiceStatus=$($COMPOSE_CMD ls --filter name=planner-airgapped | grep -o "running" | head -n 1)
    fi

    if [[ $dockerServiceStatus == "running" ]]; then
        echo "Planner Airgapped is running. Please STOP the Planner Airgapped before restoring data."
        exit 1
    fi
