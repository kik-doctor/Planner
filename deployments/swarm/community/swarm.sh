...
DOCKER_FILE_PATH=$PLANNER_INSTALL_DIR/docker-compose.yml
...
function getStackName() {
    read -p "Enter stack name [planner]: " input_stack_name
    if [ -z "$input_stack_name" ]; then
        input_stack_name="planner"
    fi
...
function syncEnvFile(){
    ...
    if [ -f "$PLANNER_INSTALL_DIR/planner.env.bak" ]; then
        ...
        value=$(getEnvValue "STACK_NAME" "$PLANNER_INSTALL_DIR/planner.env.bak")
        if [ -n "$value" ]; then
            updateEnvFile "STACK_NAME" "$value" "$DOCKER_ENV_PATH"
        fi
    fi
    echo "Environment variables synced successfully" >&2
    rm -f $PLANNER_INSTALL_DIR/planner.env.bak
}
...
function download() {
    ...
    TS=$(date +%s)
    if [ -f "$PLANNER_INSTALL_DIR/docker-compose.yml" ]; then
        mv $PLANNER_INSTALL_DIR/docker-compose.yml $PLANNER_INSTALL_DIR/archive/$TS.docker-compose.yml
    fi
    ...
    if [ "$STATUS" -eq 200 ]; then
        echo "$BODY" > $PLANNER_INSTALL_DIR/docker-compose.yml
    else
        ...
        if [ "$STATUS" -eq 200 ]; then
            ...
            mv $PLANNER_INSTALL_DIR/archive/$TS.docker-compose.yml $PLANNER_INSTALL_DIR/docker-compose.yml
            exit 1
        fi
    fi
