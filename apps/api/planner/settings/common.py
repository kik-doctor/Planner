...
# Module imports
from planner.utils.url import is_valid_url
...
# Planner Webhook Key
PLANNER_WEBHOOK_KEY = os.environ.get("PLANNER_WEBHOOK_KEY", get_random_secret_key())
...
MIDDLEWARE = [
    ...
    "planner.authentication.middleware.session.SessionMiddleware",
    ...
    "planner.middleware.request_body_size.RequestBodySizeLimitMiddleware",
    "planner.middleware.logger.APITokenLogMiddleware",
    "planner.middleware.logger.RequestLoggerMiddleware",
    "planner.app.middleware.cookie_authentication.CookieAuthMiddleware",
]
...
REST_FRAMEWORK = {
    ...
    "EXCEPTION_HANDLER": "planner.authentication.adapter.exception.auth_exception_handler",
...
if os.environ.get("ENABLE_READ_REPLICA", "0") == "1":
    ...
    # Database Routers
    DATABASE_ROUTERS = ["planner.utils.core.dbrouters.ReadReplicaRouter"]
    # Add middleware at the end for read replica routing
    MIDDLEWARE.append("planner.middleware.db_routing.ReadReplicaRoutingMiddleware")
...
STORAGES["default"] = {"BACKEND": "planner.settings.storage.S3Storage"}
...
CELERY_IMPORTS = (
    # scheduled tasks
    "planner.bgtasks.issue_automation_task",
    "planner.bgtasks.exporter_expired_task",
    "planner.bgtasks.file_asset_task",
    "planner.bgtasks.email_notification_task",
    "planner.bgtasks.cleanup_task",
    "planner.license.bgtasks.tracer",
    # management tasks
    "planner.bgtasks.dummy_data_task",
    # issue version tasks
    "planner.bgtasks.issue_version_sync",
    "planner.bgtasks.issue_description_version_sync",
)
...
SESSION_ENGINE = "planner.db.models.session"
...
CSRF_FAILURE_VIEW = "planner.authentication.views.common.csrf_failure"
