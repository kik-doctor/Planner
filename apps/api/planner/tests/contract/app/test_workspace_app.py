...
from planner.db.models import Workspace, WorkspaceMember
...
class TestWorkspaceAPI:
    ...
    @pytest.mark.django_db
    @patch("planner.bgtasks.workspace_seed_task.workspace_seed.delay")
    def test_create_workspace_valid_data(self, mock_workspace_seed, session_client, create_user):
        ...
        workspace_data = {
            "name": "planner",
            "slug": "pla-ne-test",
            "company_name": "OneWorkspaceX LLC.",
        }
    ...
    def test_create_duplicate_workspace(self, mock_workspace_seed, session_client):
        ...
        session_client.post(url, {"name": "planner", "slug": "pla-ne"}, format="json")

        # Try to create a workspace with the same slug
        response = session_client.post(url, {"name": "planner", "slug": "pla-ne"}, format="json")
