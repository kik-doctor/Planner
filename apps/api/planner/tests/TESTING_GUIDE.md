# Testing Guide for Planner

This guide explains how to write tests for Planner using our pytest-based testing strategy.
...
```python
import pytest
from planner.api.serializers import MySerializer
...
Smoke tests should be placed in `tests/smoke/` directory and use the `planner_server` fixture to test against a real HTTP server.
...
class TestCriticalFlow:
    @pytest.mark.django_db
    def test_login_flow(self, planner_server, create_user, user_data):
        # Get login URL
        url = f"{planner_server.url}/api/auth/signin/"
...
```python
from planner.tests.factories import UserFactory, WorkspaceFactory
...
python -m pytest -m unit --cov=planner
```
