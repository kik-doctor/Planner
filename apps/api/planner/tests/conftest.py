...
from planner.db.models import User, Workspace, WorkspaceMember
from planner.db.models.api import APIToken
...
def user_data():
    ...
    return {
        "email": "test@planner.oneworkspacex.com",
...
def session_client(api_client, create_user):
    """Return a session authenticated API client for app API testing, which is what planner.app uses"""
...
def create_bot_user(db):
    ...
    user = User.objects.create(
        email=f"bot-{unique_id}@planner.oneworkspacex.com",
...
@pytest.fixture
def planner_server(live_server):
    """
