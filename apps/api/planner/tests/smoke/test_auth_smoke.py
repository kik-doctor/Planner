...
class TestAuthSmoke:
    ...
    def test_login_endpoint_available(self, planner_server, create_user, user_data):
        """Test that the login endpoint is available and responds correctly"""
        # Get the sign-in URL
        relative_url = reverse("sign-in")
        url = f"{planner_server.url}{relative_url}"
...
class TestHealthCheckSmoke:
    ...
    def test_healthcheck_endpoint(self, planner_server):
        """Test that the health check endpoint is available and responds correctly"""
        # Make a request to the health check endpoint
        response = requests.get(f"{planner_server.url}/")
