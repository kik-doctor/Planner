...
This directory contains tests for the Planner application. The tests are organized using pytest.
...
Planner has two types of API endpoints:

1. **External API** (`planner.api`):
...
```bash
# Run unit tests
python -m pytest planner/tests/unit/

# Run API contract tests
python -m pytest planner/tests/contract/api/

# Run App contract tests
python -m pytest planner/tests/contract/app/

# Run smoke tests
python -m pytest planner/tests/smoke/
```
...
- `planner_server`: Live Django test server for HTTP-based smoke tests
...
2. Use the correct client fixture based on the API being tested:
   ...
   - For smoke tests with real HTTP, use `planner_server`
...
```bash
python -m pytest --cov=planner --cov-report=term --cov-report=html
```
