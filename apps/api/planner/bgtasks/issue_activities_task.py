...
# Module imports
from planner.app.serializers import IssueActivitySerializer
from planner.bgtasks.notification_task import notifications
...
from planner.utils.issue_relation_mapper import get_inverse_relation
from planner.utils.uuid import is_valid_uuid
    ...
    if current_instance.get("archived_at") != requested_data.get("archived_at"):
        if requested_data.get("archived_at") is None:
            ...
            if requested_data.get("automation"):
                comment = "Planner has archived the issue"
                new_value = "archive"
            else:
    ...
    if requested_data.get("closed_to") is not None:
        ...
        issue_activities.append(
            IssueActivity(
                ...
                project_id=project_id,
                workspace_id=workspace_id,
                comment="Planner updated the state to ",
                old_identifier=None,
