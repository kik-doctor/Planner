...
# Module imports
from planner.db.models import User, Workspace, WorkspaceMemberInvite
...
def workspace_invitation(email, workspace_id, token, current_site, inviter):
    try:
        ...
        subject = f"{user.first_name or user.display_name or user.email} has invited you to join them in {workspace.name} on planner"  # noqa: E501
        ...
        msg.send()
        logging.getLogger("planner.worker").info("Email sent successfully")
        return
